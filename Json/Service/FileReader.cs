...
public class FileReader
{
    ...
    public FileReader(ReaderOptions options) { 
        _options = options;
    }
    ////Путь файла
    ...
    public List<FileInfo> GetFileAll() //используется для получения списка файлов в сетевой папке.
        => Directory.GetFiles(networkFolderSettings) //для получения списка файлов в сетевой папке, указанной в настройках.
    ...
    public List<FileInfo> GetNotImportedFiles() 
    {
        ...
        foreach (FileInfo file in allFiles)
        {
            ...
            var findName = db.historyFiles.FirstOrDefault(f => f.Name == file.Name);
            ...
            if (findName != null)
            {
                ...
                var findImport = db.historyFiles.Where(i=>i.import == fileCheckImport.noImport).FirstOrDefault(n=>n.Name == file.Name);
                if (findImport != null) {
                    result.Add(file);
                }
            }
